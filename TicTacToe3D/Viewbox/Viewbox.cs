...
// Edits by Alan Mendelevich (2009-12-05):
// Minimized dependencies by:
// - replacing exception message strings with hardcoded text
// - replacing IsZero() extension method calls with local implementation
    ...
    public sealed partial class Viewbox : ContentControl
    {
        ...
        private Size ComputeScaleFactor(Size availableSize, Size contentSize)
        {
            ...
            if ((stretch != Stretch.None) && (isConstrainedWidth || isConstrainedHeight))
            {
                ...
                scaleX = Math.Abs(contentSize.Width) < 2.2204460492503131E-15 ? 0.0 : (availableSize.Width / contentSize.Width);
                scaleY = Math.Abs(contentSize.Height) < 2.2204460492503131E-15 ? 0.0 : (availableSize.Height / contentSize.Height);
